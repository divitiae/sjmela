...
class SearchScreen extends Component {
  ...
  _handleSearch = async () => {
    ...
    navigation.push(screen, { _results });
  };
      ...
      <GooglePlacesAutocomplete
        placeholder={
          listingFilters.length > 0
            ? listingFilters.filter((item) => item.key === "google_place")[0]
                .name
            : ""
        }
        ...
        query={{
          key: settings.oGoogleMapAPI.key,
          language: "en",
          types: settings.oGoogleMapAPI.types,
        }}
        styles={{
          container: {
            padding: 0,
  ...
  _handleDateToChange = (key) => (date) => {
    // console.log(this.state.dateFrom, this.state.results);
    this.setState(
      {
        dateTo: date,
        results: {
          ...this.state.results,
          [key]: {
            from: moment(this.state.dateFrom).format("MM/DD/YYYY"),
            to: date,
          },
        },
      },
